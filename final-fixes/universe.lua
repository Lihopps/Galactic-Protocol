...
local function add_moon_to_game(moons)
    for _, moon_data in pairs(moons) do
        data:extend({ moon_data.moon, {
            type = "space-connection",
            name = moon_data.parent .. "-to-" .. moon_data.moon.name,
            subgroup = moon_data.moon.subgroup,
            from = moon_data.parent,
            to = moon_data.moon.name,
            order = moon_data.parent .. "z" .. moon_data.moon.order,
            need_spanwdef = true
        } })
    end
end
...
local function create_universe()
    ...
    for _, planet in pairs(gpPlanetCollector.modded.planet) do
        uCreator.switch_planet(gen, galaxy_objects, planet)
    end
    ...
    add_moon_to_game(gpmoontracker)
